<template>
  <div>
    ...
    <div>
      <h4>
        点我调用
        ​/shop-service​/v1​/api-test​/path​/demo1​/username​/{username}​/password​/{password}接口
      </h4>
...
<script>
// 导入外部定义的接口函数
import { postDemo1, pathDemo1, postDemo2 } from "../api/api-test.js";
export default {
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    async handleClick3() {
      ...
      /* let result = await this.$http({
        ...
        console.log("完了出错了", err);
      });
      console.log("result", result.data.data.list); */
      let result = await postDemo2("admin", "123456").catch((err) =>
        console.log("出错了", err)
      );
      console.log("result", result.data.data.list);
    },
