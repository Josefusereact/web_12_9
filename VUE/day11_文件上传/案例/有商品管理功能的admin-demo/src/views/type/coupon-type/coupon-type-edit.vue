<template>
  <div>
      <el-page-header @back="handleBack" content="编辑优惠券类型信息"></el-page-header>
      <el-row>
			<el-col :span="6">
				<el-form 
					:model="addForm" 
...
<script>
import {getCouponTypeById,updateCouponType} from '../../../api/coupon-type-api'
export default {
    data(){
        return {
            addForm:{
    ...
    methods:{
        async handleAdd(){
            ...
            if(isValid){
                //调用保存接口，并跳转回到查询页面
                await updateCouponType(this.addForm);
                this.handleBack();
            }
    ...
    async created(){
        ...
        let addForm = await getCouponTypeById(id);
