  ...
  <div class="page">
       <p-page-header title="卡类型管理"></p-page-header>
		<el-form inline :model="queryForm">
			<el-form-item size="mini" label="卡类型名称">
				<el-input placeholder="请输入卡类型名称" clearable v-model="queryForm.name"></el-input>
			</el-form-item>
			<el-form-item size="mini" label="卡类型编码">
				<el-input placeholder="请输入卡类型编码" clearable v-model="queryForm.bm"></el-input>
			</el-form-item>
			<el-form-item size="mini" label="备注">
				<el-input placeholder="请输入备注" clearable v-model="queryForm.remark"></el-input>
			</el-form-item>
		...
		<el-table
			...
			<el-table-column label="昵称" prop="remark"></el-table-column>
...
export default {
    ...
    computed:{
        ...
        formatTime(){
            return (inserttime)=>{
				let time = new Date(inserttime);
                return time.getFullYear()+"年" + (time.getMonth()+1)+'月';
            }
        }
    },
    methods:{
        //列表页面用到了俩个actions里面的函数，1. 查询列表， 2删除记录
        ...
        // 点击查询按钮触发的⌚️
			...
			async handleRemove(id){
				// 通过同步化写法调用$confirm方法
				let confirm =await  this.$confirm('正在删除优惠券类型','提示',{
					type:'warning'
				}).catch(err => err)
				if(confirm == 'confirm'){
					// 如果点击确定
