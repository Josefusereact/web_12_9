  ...
  <div>
    ...
    <el-table :data="list" size="mini" border>
      ...
      <el-table-column label="操作" width="200px">
        <template v-slot="{ row }">
          <el-button type="danger" size="mini" @click="handleEdit(row.id)"
            >修改</el-button
          >
          <el-button
            type="danger"
...
export default {
  ...
  methods: {
    ...
    async handleDelete(id, username) {
      let isOk = this.$confirm(
        `此操作将永久删除该<b class="red">${username}</b>用户, 是否继续?`,
        "提示",
        {
      ...
      if (isOk == "confirm") {
        ...
        this.getListForPage(this.queryForm);
      }
    },
  },
