...
export default {
  ...
  computed: {
    ...mapState("goodsTypeWeb2003Model", ["list", "page"]),
    formatTime() {
      return (insertTime) => {
  ...
  methods: {
    ...mapActions("goodsTypeWeb2003Model", ["getListForPage"]),
    // 点击查询按钮触发的⌚️
    async handleClick() {
    ...
    // 跳转到添加数据的页面
    handleAdd() {
      this.$router.push("/goods-type-add-web2003");
    },
    // 删除数据调用的方法
    async handleRemove(id) {
      ...
      if (confirm == "confirm") {
        // 如果点击确定
        //首先调用删除业务
        await this.deleteGoodsTypeById(id);
    ...
    // 跳转到编辑用户的页面
    handleEdit(id) {
      this.$router.push({ path: "/goods-type-edit-web2003", query: { id } });
    },
  },
  async created() {
    await this.getListForPage(this.queryForm);
