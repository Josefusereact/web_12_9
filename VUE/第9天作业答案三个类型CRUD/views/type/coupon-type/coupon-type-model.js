...
export default {
    ...
    actions:{
      ...
      async getListForPage({commit},queryForm){
        let res = await getCouponTypeList(queryForm).catch(err=>err);//res是response响应的意思
      ...
      async insert({commit},addForm){
        await addCouponType(addForm).catch(err => err);
      },
      async findById({commit},id){
        let res = await getCouponTypeById(id).catch(err => err);//返回的json里包含我们要得对象
        if(res.data.code == 200){
            return res.data.data;//这个最后的data代表取出来的商品类型
        }
      },
      async update({commit},addForm){
        await updateCouponType(addForm).catch(err => err)
      },
      async deleteById({commit},id){
        await deleteCouponTypeById(id).catch(err=>err);
      },
    }
  }