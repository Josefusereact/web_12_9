...
> js的异步历史：
>
> 在过去的js代码中如果代码中包含异步的场景，比如setTimeout或setInterval时我们可以根据他的回调函数来直接的处理异步代码完成后的事情，由于产生异步的代码一定是在同步代码之后发生，这时我们无法使用顺序执行的同步代码获取异步代码的结果，就需要在异步的流程上使用回调函数来等待异步代码完成之后的通知。
>
> 也就导致了我们在同步代码中如果涉及到异步的流程就需要使用回调函数的嵌套处理
...
如果有多重的异步代码需要按顺序来执行我们只能在每一层异步代码的回调中去进行下一步操作来保证之前的异步流程是有序的

```js
//test为异步代码，通过回调保证每次的异步代码都在上一次的结果中触发
test(function(){
  test(function(){
     test(function(){
    		test(function(){
    			//...最终的回调函数保证能拿到之前所有的异步代码的结果
  			})
  	 })
  })
})
...
> 以上的场景就是由于过去的回调结构导致的函数嵌套层数过多，我们无法通过代码直观的来观察函数的执行顺序，降低了可读性，这就是我们平时所说的回调地狱

**Promise如何解决的回调地狱**

promise是通过在每次定义一个未知结果的对象，来将这个对象放到我们的同步结构中，用以来监听未来的结果，当未来的异步代码执行完毕之后无论是成功或者是失败都会触发这个对象的.then函数中的回调函数。通过这种方式，将代码的结构从异步嵌套变成.then函数的链式调用来将原本所有回调函数的嵌套变成上下结构的书写方式

比如

```js
let p = new Promise(function(resolve,reject){
  //在适合的情况下使用resovle来触发Promise的内部状态的改变
  //这时p对象的.then中的回调函数就会执行
  resolve()
})
//p.then同时会返回一个promise对象所以如果连续的回调可以
//以如下的形式书写
p.then(function(){
  ....
})
...
根据上面的结构得知promise将过去的嵌套回调函数的方式变成了链式调用的回调方式，让我们的异步代码既能保证有序的执行，又能按照上下的顺序编写来保证异步的流程和代码的可读性
...
> 构造函数`Promise`必须接受一个函数作为参数，我们称该函数为`handle`，`handle`包含`resolve`和`reject`两个参数，它们是两个函数。

**这里结合案例中的[demo1.html](../案例/demo1.html)来查看Promise的基本用法和链式调用**
...
看下面的例子有resovle还有reject，只能变为一个状态，reject没有执行

**这里我们结合案例中的[demo2.html]()来看一下Promise的状态案例**
...
2、如果 `onFulfilled` 或者`onRejected` 抛出一个异常 `e` ，则 `promise2` 必须变为失败`（Rejected）`，并返回失败的值 `e`，以下代码是demo4.html文件：
...
//在promise的.then中如果状态为成功，且当前的promise.then中传入的不是函数,这个例子是字符串 ，就会让promise跳过当前的.then直接执行下一个.then,然后下一个.then的回调函数的参数值还是上一个.then中的参数
...
**下面的实现原理作为了解即可不需要掌握如何实现，我们在这里直接跳到下一个other-day-07-1的文件中**
